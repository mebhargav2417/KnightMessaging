...
public class SalesDataManagement {
	...
	@Path("/view")
	@Consumes(MediaType.APPLICATION_JSON)
	public Response viewSales(ViewRequestInput viewRequestInput){
		...
		try{
			...
			if(dbConnection != null) {
				...
				if(viewRequestInput.getDateInterval().isEmpty() || viewRequestInput.getDateInterval().equalsIgnoreCase("no") || viewRequestInput.getDateInterval().equalsIgnoreCase("false")) {
					String query = "select * from " + ConstantValues.SalesTable + " where userId = '" + viewRequestInput.getUserId() +"' LIMIT 5";
					...
					String query = "select * from " + ConstantValues.SalesTable + " where userId = '" + viewRequestInput.getUserId() +"' AND date_invoice between '" + viewRequestInput.getFromDate()
