...
public class NPDataManagement {
		...
		try{
			...
			if(dbConnection != null) {
				...
				String sql = "insert into " + ConstantValues.NetProfitTable + " values ('" + netProfit.getUserId() + "','" + netProfit.getSalesValue() + "','" + netProfit.getIncomeValue() + "','" + netProfit.getPurchaseValue() + "','" + netProfit.getExpenseValue() + "','"
						+ netProfit.getDirectExpense() + "','" + netProfit.getIndirectExpense() + "','" + netProfit.getNetProfit() + "','" + netProfit.getQuarter() + "','" + netProfit.getYear() + "','" +  new java.sql.Date(cal.getTimeInMillis()) + "','" + netProfit.getUpdatededBy() +"')";
	...
	@Path("/view")
	@Consumes(MediaType.APPLICATION_JSON)
	public Response viewNetProfit(ViewRequestInput viewRequestInput){
		...
		try{
			...
			if(dbConnection != null) {
				...
				if(viewRequestInput.getDateInterval().isEmpty() || viewRequestInput.getDateInterval().equalsIgnoreCase("no") || viewRequestInput.getDateInterval().equalsIgnoreCase("false")) {
					String query = "select * from " + ConstantValues.NetProfitTable + " where userId = '" + viewRequestInput.getUserId() +"' LIMIT 5";
					...
					String query = "select * from " + ConstantValues.NetProfitTable + " where userId = '" + viewRequestInput.getUserId() +"' AND date_invoice between '" + viewRequestInput.getFromDate()
